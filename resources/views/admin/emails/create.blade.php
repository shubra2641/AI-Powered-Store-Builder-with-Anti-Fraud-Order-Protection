...
<div class="mb-5">
    ...
    <h2 class="fs-xl font-800 m-0 gradient-text">{{ __('admin.add_template') }}</h2>
</div>
        ...
        <div class="col-span-12 col-span-9">
            <div class="glass-card p-5">
                <div class="ds-form-group mb-4">
                    ...
                    <input type="text" name="name" required class="input-premium" placeholder="e.g. Marketing Promotion">
                </div>

                <div class="grid grid-cols-12 gap-4">
                    <div class="col-span-12 col-span-6">
                        <div class="ds-form-group mb-4">
                            <label class="form-label-premium">{{ __('admin.template_slug') }} ({{ __('admin.optional') }})</label>
                            <input type="text" name="slug" class="input-premium" placeholder="marketing-promo">
                        </div>
                    </div>
                    <div class="col-span-12 col-span-6">
                        <div class="ds-form-group mb-4">
                            <label class="form-label-premium">{{ __('admin.language') }}</label>
                            <select name="language_id" required class="input-premium no-appearance">
                ...
                <div class="ds-form-group mb-4">
                    ...
                    <input type="text" name="subject" id="template_subject" required class="input-premium" placeholder="Welcome to our platform!">
                </div>

                <div class="ds-form-group mb-4">
                    <label class="form-label-premium">{{ __('admin.template_content') }} (HTML Supported)</label>
                    <textarea name="content" id="template_content" rows="12" required class="input-premium" placeholder="<h1>Hello @{{ $user->name }}</h1>..."></textarea>
                </div>

                <div class="ds-form-group mb-4">
                    <label class="form-label-premium">{{ __('admin.description') }}</label>
                    <textarea name="description" rows="2" class="input-premium"></textarea>
                </div>

                <div class="d-flex justify-end gap-3 mt-4">
                    <a href="{{ route('admin.emails.index') }}" class="btn-dark">{{ __('admin.cancel') }}</a>
                    <button type="submit" class="btn-gradient px-5">{{ __('admin.save') }}</button>
                </div>
            </div>
        </div>
